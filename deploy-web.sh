...
BUCKET=$(aws-vault exec "$VAULT_PROFILE" -- terraform -chdir="${REPO_ROOT}/iac/environments/${ENV}" output -raw static_site_bucket_name)
DISTRIBUTION_ID=$(aws-vault exec "$VAULT_PROFILE" -- terraform -chdir="${REPO_ROOT}/iac/environments/${ENV}" output -raw static_site_distribution_id)
