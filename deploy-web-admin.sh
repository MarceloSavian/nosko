...
BUCKET=$(aws-vault exec "$TF_PROFILE" -- terraform -chdir="${REPO_ROOT}/iac/environments/${ENV}" output -raw admin_site_bucket_name)
DISTRIBUTION_ID=$(aws-vault exec "$TF_PROFILE" -- terraform -chdir="${REPO_ROOT}/iac/environments/${ENV}" output -raw admin_site_distribution_id)
