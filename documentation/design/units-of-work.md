...
| U3 | Auth & Household | signup/verify/login/MFA (remember device)/sessions; the `auth_tokens`/`user_sessions`/`household_invitations` repositories (deferred from U2, owned here alongside their use-cases); create/invite/accept (max 2 members, already DB-enforced); SES verification; account visibility (E1, E2) | P1 | U2 |
