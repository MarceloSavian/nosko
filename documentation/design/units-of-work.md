...
| U7 | Shared payments ✅ | `payments.*` RPC (list/create/update/remove/summary) + `payments.exportCsv` HttpApi over `shared_payments` (household-scoped, joint/shared accounts only); base-currency amount converted and stored once at confirmation (`FxConversion`, real `NoFxRate` on a missing rate, not swallowed); cycle resolved from the booked date; `computeByCategory` for spend-vs-cap; `CycleEngine.variableTotal` now real (was 0 since U6) (E7) | P1 | U6 |
...
- U2 ✅: `pnpm verify:migrations` (pglite) — `app_role` has `rolsuper = rolbypassrls = false`; a
